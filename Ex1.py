...
def get_datasets():
    ...
    S_db_images = np.array(images_coarse)
    S_db_poses = np.array(poses_coarse)
    ...
    S_test_images = np.array(S_test_images)
    S_test_poses = np.array(S_test_poses)
    
    '''missing normalization of the RGB channels'''
    
    return S_train_images, S_train_poses, S_test_images, S_test_poses, S_db_images, S_db_poses
...
def batch_generator(train_images, train_poses, db_images, db_poses, batch_size, plot = False):
    ...
    triplet_images = np.array(triplet_images).reshape((1, -1, 64, 64, 3))
